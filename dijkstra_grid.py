...
def algorithm(start, finish, m_grid_to_graph):
    count_node = len(m_grid_to_graph)
    ...
    while finish not in closed:
        for i in range(count_node):
            ...
            for node_id in range(count_node):
                if m_grid_to_graph[index_min_cost][node_id] == 1:  # если связь существует
                    if cost[index_min_cost] + m_grid_to_graph[index_min_cost][node_id] < cost[node_id]:  # если стоимость меньше
                        cost[node_id] = cost[index_min_cost] + m_grid_to_graph[index_min_cost][node_id]  # обновить стоимость
                        pathes[node_id] = str(pathes[index_min_cost]) + ' ' + str(node_id)  # обновить путь
