...
def algorithm(start, cost):
    ...
    open = [start]  # добавляем начальную вершину в очередь
...
def main(start_num):
    ...
    while start_num != let_to_num[finish]:
        if start_num not in open and start_num not in closed:
            pathes[let_to_num[start]] = start
            algorithm(start_num, cost)
        start_num += 1
    return cost[start_num]